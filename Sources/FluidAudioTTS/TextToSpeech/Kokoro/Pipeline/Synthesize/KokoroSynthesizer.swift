...
public struct KokoroSynthesizer {
    ...
    /// Shared time stretcher instance for voice speed adjustment
    private static let timeStretcher = TimeStretcher(config: .speech)

    /// Adjust playback speed using high-quality time-stretching (WSOLA algorithm).
    /// Unlike naive sample manipulation, this preserves pitch at all speeds.
    /// - Parameters:
    ///   - samples: Input audio samples
    ///   - factor: Speed factor (2.0 = 2x faster, 0.5 = half speed)
    /// - Returns: Time-stretched audio samples with preserved pitch
    private static func adjustSamples(_ samples: [Float], factor: Float) -> [Float] {
        ...
        // Use WSOLA time-stretching for high-quality speed adjustment
