...
enum KokoroChunker {
    ...
    // MARK: - Prosody-Aware Pause Calculation

    /// Calculate pause duration (in ms) based on trailing punctuation of a chunk.
    /// Used between chunks to create natural speech rhythm.
    private static func calculatePauseAfterChunk(_ text: String) -> Int {
        let trimmed = text.trimmingCharacters(in: .whitespaces)
        guard let lastChar = trimmed.last else { return 0 }

        // Check for ellipsis at end
        if trimmed.hasSuffix("...") || trimmed.hasSuffix("…") {
            return 700  // Ellipsis - thoughtful pause
        }

        switch lastChar {
        case ".", "。":  // Period (including CJK)
            return 500
        case "?", "？":  // Question mark
            return 550
        case "!", "！":  // Exclamation
        ...
        case "—", "–":  // Em/en dash
            return 250
        default:
            return 0  // No pause for other endings
        }
    }
    ...
    private static func buildChunks(
        ...
        func flushChunk() {
            ...
            // Calculate prosody-aware pause based on trailing punctuation
            let pauseMs = calculatePauseAfterChunk(textValue)
    ...
    private static func resolvePhonemes(
        ...
        if phonemes == nil, let ipa = try await EspeakG2P.shared.phonemizeWithCache(word: normalized) {
            ...
            !spelledTokens.isEmpty
        {
            ...
            for spelled in spelledTokens {
                ...
                if segment == nil, let ipa = try await EspeakG2P.shared.phonemizeWithCache(word: spelled) {
                    let mapped = PhonemeMapper.mapIPA(ipa, allowed: allowed)
                    if !mapped.isEmpty {
                        segment = mapped
                    }
